...
def is_disposable(email, disposable_domains):
    if not re.match(EMAIL_REGEX, email):
        return False
    # split email to check domain against blacklist
...
def process_email(input_file):
    try:
        ...
        # Get operating system and determine root directory path
        if os.name == "nt":  # Windows
            root_dir_path = "C:\\"
        else:  # macOS/Linux
            root_dir_path = "/"

        # Create a subdirectory within the root directory for better organization (replace with your desired name)
        output_dir = os.path.join(root_dir_path, "Validated_Emails")
        os.makedirs(output_dir, exist_ok=True)  # Create output directory if it doesn't exist

        # Define output file paths on the desktop
        disposable_file_path = os.path.join(output_dir, 'DisposableEmails.xlsx')
        valid_file_path = os.path.join(output_dir, 'ValidEmails.xlsx')
        ...
        messagebox.showinfo(title="Success",
                            message="Processing complete. Saved as 'DisposableEmails.xlsx' and 'ValidEmails.xlsx'.")
